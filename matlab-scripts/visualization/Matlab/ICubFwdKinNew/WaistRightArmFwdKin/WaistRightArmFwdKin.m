...
% Usage:
%
%       [T_Ro0, T_0n] = WaistLeftArmFwdKin(wtheta,rtheta)
%
% Input: 
%
...
function [T_Ro0, T_0n, J, G_sL8, G_sL10] = WaistRightArmFwdKin(wtheta,rtheta, display)
...
% G_34=subs(G_LtoL, {a d alph thet}, {    0 -107.74     pi/2    theta0-pi/2});
% G_45=subs(G_LtoL, {a d alph thet}, {    0       0    -pi/2    theta1-pi/2});
% G_56=subs(G_LtoL, {a d alph thet}, {    0 -152.28    -pi/2    theta2-pi/2-15*pi/180}); 
% G_67=subs(G_LtoL, {a d alph thet}, {   15       0     pi/2    theta3}); 
% G_78=subs(G_LtoL, {a d alph thet}, {    0  -137.3     pi/2    theta4-pi/2}); 
% G_89=subs(G_LtoL, {a d alph thet}, {    0       0     pi/2    theta5+pi/2}); 
% G_910=subs(G_LtoL, {a d alph thet},{62.5      16        0    theta6 + pi}); 
...
G_910=evalDHMatrix( 62.5*k,      16*k,        0,    theta6+pi); 
...
%(G_01*G_12*G_23*G_34*G_45*G_56*G_67*G_78*G_89)^(-1)
%(G_01*G_12*G_23*G_34*G_45*G_56)^(1)
...
if (display==1)
    ...
    % %
    cy0 = DrawCylinder(-107.74*k, 20*k, G_sL3, LinkColor3, 100, 0.4);
    cy0 = DrawCylinder(-152.28*k, 10*k, G_sL5, LinkColor3, 100, 0.4);
    cy1 = DrawCylinder( -137.3*k,  5*k, G_sL7, LinkColor3, 100, 0.4);
    % Palm
