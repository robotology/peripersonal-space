...
%
%    E:\Dropbox\PhD_IIT\1st Year\iCubSkin\left_forearm_V1\taxels1D_learned_l_forearm.ini
%
...
%         figure;
%         bar(D,P(i,:));
%         title(int2str(l_forearm(i)));
%         grid on;
%         hold on;
% %         [f,xi] = ksdensity([D,P(i,:),D,'Bandwidth',4*d,'NumPoints',numPts]);
%         [f,xi] = ksdensity([P(i,:)],D,'Bandwidth',4*d);
% %         hist(P(i,:));
%         plot(xi,f/max(f),'r')
    
%     f = interp1(D,P(i,:),xi,'pchip');
%     plot(xi,f,'r')
        
%         hold off;

%         f(i) = figure; parzen_estimation(D,P(i,:),4*d,'r',int2str(l_forearm(i)));
%         grid on
        if (mod(j,4)==1)
            fig(i) = figure; 
...
%         hist_map3d([0 0 0],x,f(i,find(x>0)));
        for j=1:M
            if (l_forearm(i)==j)
                h = hist_map3d([taxel_pos(j,1),taxel_pos(j,2),taxel_pos(j,3)],-sign(j-192.5)*x(70:end),-f(i,70:end),100,0.1);
